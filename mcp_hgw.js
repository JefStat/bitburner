...
async function runHGW(target) {
	...
	const batch = getHostsAndThreadsFunc(hackThreadsNeeded);
...
function getHostsAndThreadsFunc(hackThreadFunction, growThreadFunction, weakThreadFunction) {
	...
	while (strength > 0) {
		...
		weakenHost = getHostAndThreadsFunc(weaken_scriptRam, weakThreadFunction, strength);

		if (!hackHost.host || !growHost.host || !weakenHost.host ){
			strength -= .25;
	...
	return {hackHost, growHost, weakenHost};
}
function getHostAndThreadsFunc(scriptRam, threadFunction, strength) {
	...
	hosts.sort((a, b) => a.threadsAvailMinusNeed - b.threadsAvailMinusNeed);
	// take the closet fit from front of the array or empty caller needs to adjust thread ask
