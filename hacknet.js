...
const argsSchema = [
    ['max-payoff-time', '1h'], // Controls how far to upgrade hacknet. Can be a number of seconds, or an expression of minutes/hours (e.g. '123m', '4h')
...
export function upgradeHacknet(ns, maxSpend, maxPayoffTimeSeconds = 3600 /* 3600 sec == 1 hour */, options) {
    ...
    const upgrades = [{ name: "none", cost: 0 }, {
        ...
        addedProduction: nodeStats => ns.formulas.hacknetServers.hashGainRate(nodeStats.level + 1, 0, nodeStats.ram, nodeStats.cores, currentHacknetMult)
    }, {
        name: "ram", upgrade: ns.hacknet.upgradeRam, cost: i => ns.hacknet.getRamUpgradeCost(i, 1), nextValue: nodeStats => nodeStats.ram * 2,
        addedProduction: nodeStats => ns.formulas.hacknetServers.hashGainRate(nodeStats.level, 0, nodeStats.ram * 2, nodeStats.cores, currentHacknetMult)
    }, {
        name: "cores", upgrade: ns.hacknet.upgradeCore, cost: i => ns.hacknet.getCoreUpgradeCost(i, 1), nextValue: nodeStats => nodeStats.cores + 1,
        addedProduction: nodeStats => ns.formulas.hacknetServers.hashGainRate(nodeStats.level, 0, nodeStats.ram, nodeStats.cores + 1, currentHacknetMult)
    }, {
